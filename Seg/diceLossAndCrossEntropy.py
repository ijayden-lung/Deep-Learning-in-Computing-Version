...
def loss_fn(y_pred, y):
    ...
    crossEntropy =  nn.CrossEntropyLoss()(y_pred, y.long())
    ...
    y = F.one_hot(y.long(), num_classes = 21).float()
    #y_pred   = F.one_hot(y_pred, num_classes = 21).float()
    ...
    dice_loss =  1 - ((2. * intersection + smooth)/(torch.sum(y_pred) + torch.sum(y) + smooth))

    loss = 0.5* crossEntropy + 0.5*dice_loss
...
model.load_state_dict(torch.load('seg_model_MaxPool_diceLoss_gpu[0, 1, 2, 3]_n200_bs8_lr0.001.pkl'))
...
run_id = 'seg_model_MaxPool_diceLoss_gpu{}_n{}_bs{}_lr{}'.format(gpu_id, epochs, batch_size, learning_rate); print('\n\nTraining', run_id)
